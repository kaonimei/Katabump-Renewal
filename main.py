...
def _goto_server_detail(sb) -> bool:
    ...
    # 通过 JavaScript 精准查找表格中文字为 'see' 的项并点击
    clicked = sb.execute_script("""
        return (function() {
            var elements = document.querySelectorAll('a, button, span, td');
            for (var i = 0; i < elements.length; i++) {
                var el = elements[i];
                var txt = (el.textContent || '').trim().toLowerCase();
                if (txt === 'see') {
                    var parentLink = el.closest('a');
                    var target = parentLink || (el.tagName === 'A' ? el : el.querySelector('a') || el);
            ...
            // 兜底：查找包含 /servers/ 的表格行链接
            var links = document.querySelectorAll('table a[href*="/servers/"]');
...
def _submit_renew(sb):
    """点击弹窗内的 Renew，并等待其内置的轻量异步验证（Vérification...）与提交完成"""
    print("🖱️  点击弹窗中的 Renew 确认按钮...")
    time.sleep(1)
    ...
    success = sb.execute_script("""
        ...
        for (var i = 0; i < btns.length; i++) {
            var text = (btns[i].textContent || '').trim().toLowerCase();
            if (text.includes('renew') || text.includes('vérification')) {
                btns[i].click();
                return true;
            }
        }
    ...
    if not success:
        print("⚠️ 未能在弹窗内通过文本找到 Renew，尝试通用兜底点击...")
        try:
            submit = sb.find_element('div.modal.show button.btn-primary', timeout=3)
            submit.click()
        except Exception:
            pass

    # 动态等待内置验证（Vérification... 进度条）完成、弹窗自动关闭或结果出现
    print("⏳ 等待后台验证与异步提交完成...")
    for i in range(20):
        time.sleep(1)
        modal_still_show = sb.execute_script("return document.querySelector('div.modal.show') !== null;")
        alert_text = _read_alert(sb)
        
        if alert_text or not modal_still_show:
            print(f"✅ 验证/提交已完成（耗时 {i+1}s）")
            break
