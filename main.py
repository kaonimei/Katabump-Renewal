...
def _goto_server_detail(sb) -> bool:
    ...
    try:
        # 精准匹配形如 /servers/edit?id= 的链接
        links = sb.find_elements('a[href*="servers/edit?id="]')
        target_url = ""
        for lnk in links:
        ...
        # 兜底：查找任何包含 servers 的链接
        if not target_url:
            for lnk in sb.find_elements('a[href*="servers"]'):
                href = lnk.get_attribute("href") or ""
                if "id=" in href and "/create" not in href:
                    target_url = href
                    break

        if target_url:
            print(f"🔗 捕获到目标链接: {target_url}")
            sb.open(target_url)
        else:
            print("❌ 未能在页面中找到匹配的服务器编辑链接")
            sb.save_screenshot("server_link_fail.png")
            return False
